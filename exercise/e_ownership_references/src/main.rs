...
fn main() {
    ...
    fn inspect(command_line_arg: &String) {
      if command_line_arg.ends_with("s") {
        println!("plural");
        return
      }
      println!("singular");
    ...
    fn change(command_line_arg: &mut String) {
      if !command_line_arg.ends_with("s") {
        command_line_arg.push_str("s")
      }
    }
    ...
    fn eat(command_line_arg: String) -> bool {
      if command_line_arg.starts_with("b") && command_line_arg.contains("a") {
        return true;
      }
      return false;
    }
    ...
    fn bedazzle(command_line_arg: &mut String) {
      *command_line_arg = "sparkly".to_string();
    }
}
